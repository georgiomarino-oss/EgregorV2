...
function parseVoices(input) {
  ...
  const parsed = input
    ...
    .filter(Boolean)
    .map((part) => {
      const [left, right] = part.includes(':') ? part.split(':', 2) : [part, part];
...
async function invokeGeneratePrayerAudio({
  ...
  const response = await requestJson(`${supabaseUrl}/functions/v1/generate-prayer-audio`, {
    method: 'POST',
    headers: {
      apikey: serviceRoleKey,
      Authorization: `Bearer ${serviceRoleKey}`,
...
async function main() {
  ...
  console.log(`Voices: ${voices.map((voice) => `${voice.label}:${voice.id}`).join(', ')}`);
  ...
  const filteredScripts = scripts
    .filter((script) => {
      if (!script?.id || !script?.prayer_library_item_id || typeof script.script_text !== 'string') {
        return false;
      }
    ...
    .map((script) => ({
      ...
      language: typeof script.language === 'string' && script.language.trim() ? script.language : 'en',
      prayerLibraryItemId: script.prayer_library_item_id,
      prayerLibraryScriptId: script.id,
      scriptText: script.script_text.trim(),
      title: publicItemById.get(script.prayer_library_item_id)?.title?.trim() || null,
    }))
  ...
  console.log(`Prewarm complete. Success: ${successCount}. Failed: ${failureCount}.`);
  if (failures.length > 0) {
    console.log('Failure summary:');
    failures.slice(0, 20).forEach((entry) => {
      console.log(`- ${entry.title ?? 'Untitled'} (${entry.voice}): ${entry.message}`);
    });
  }
}
